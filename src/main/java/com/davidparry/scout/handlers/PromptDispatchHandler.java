    ...
    public HandlerResponse handle(JsonRpcRequest request) {
        if (request == null || request.params() == null || request.params().arguments() == null) {
            ...
            if (request == null) {
                ...
                return new HandlerResponse(new PromptResponse(errorMsg, Collections.emptySet()));
            }
        }
        ...
        if (prompt == null) {
            ...
            return new HandlerResponse(new PromptResponse("No prompt for the following named prompt " + name, Collections.emptySet()));
        } else {
            Set<PromptMessage> messages = prompt.build(arguments);
            return new HandlerResponse(new PromptResponse(prompt.description(), messages));
        }
    }
}
